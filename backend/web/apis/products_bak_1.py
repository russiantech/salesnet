...
# getting a product by_id / slug already handled in 1st resource though.
@product_bp.route('/products/<product_id>/product', methods=['GET'])
@jwt_required()
def by_id(product_id):
    try:
        product = Product.query.get_or_404(product_id)
        return success_response("Product fetched sucessfully", data=product.get_summary())
    except Exception as e:
        # Handle any unexpected errors
        return error_response(f"An error occurred: {str(e)}", status_code=500)
...
def by_slug(product_slug):
    try:
        ...
        product = Product.query.filter(slug=product_slug).first()
        if product is None:
            return error_response("Product not found.", status_code=404)
        return success_response("Products fetched successfully.", data=product.get_summary())
    except Exception as e:
        # Handle any unexpected errors
        traceback.print_exc()
        return error_response(f"An error occurred: {str(e)}", status_code=500)

@product_bp.route('/products/<user_id>/user', methods=['GET'])
...
def by_user(user_id):
    try:
        ...
        Parameters:
        - user_id: The ID of the user to filter products.
        """
        ...
        products = Product.query.filter(
            Product.users.any(id=user_id)
            ).order_by(desc(Product.created_at)
            ).paginate(page=page, per_page=page_size)

        # Serialize the paginated result using PageSerializer
        data = PageSerializer(pagination_obj=products, resource_name="products", context_id=user_id, include_user=True)
        # data = PageSerializer(pagination_obj=products, resource_name="products")
        data = data.get_data()
...
def by_category(category_id):
    try:
        ...
        # Serialize the paginated result using PageSerializer
        data = PageSerializer(pagination_obj=products, resource_name="products", context_id=category_id)
        data = data.get_data()
...
def create():
    try:
        ...
        # Here we check if the error is related to a duplicate entry for 'slug'
        if 'Duplicate entry' in str(e.orig):
            return error_response("Duplicate entry for slug. Product already exists.", status_code=409)
        else:
            # Handle other integrity errors (e.g., foreign key constraint)
